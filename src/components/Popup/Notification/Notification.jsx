...
import { useMarkReadQuery } from "../../../redux/services";
            ...
            <div className="border border-b-grey-input rounded-xl mx-[26px] p-[20px] my-[21px]" key={index}>
              <div className="flex justify-between items-center">
                <div className="flex gap-2 items-center">
                  ...
                  <p className="text-[12px] font-semibold text-grey-label">{item?.name}</p>
                </div>
                {!item?.read && (
                  <div className="flex gap-5">
                    <p className="text-[#758089] text-[12px] cursor-pointer hover:text-primary" onClick={() => handleMarkRead(item?.id)}>
                      Mark as read{" "}
                    </p>
